class HoldingsController < ApplicationController
    ...
    def create
        ...
        @holding = @user.holdings.find_by(stock_symbol: holding_params[:stock_symbol])
        if @holding != nil
        @holding.update(quantity: (holding_params[:quantity] + @holding[:quantity]))
        else
        @holding = Ownedstock.create(
            user_id: holding_params[:user_id],
        stock_id: holding_params[:stock_id],
        quantity: holding_params[:quantity],
        stock_symbol: holding_params[:stock_symbol],
        )
        end
        @cost = holding_params[:quantity] * params[:price]
    ...
    def update
        @holding = Ownedstock.find(holding_params[:id])
        ...
        @user = User.find(holding_params[:user_id])
        @revenue = holding_params[:quantity] * params[:price]
